...
@Component({
  ...
  templateUrl: './prev-calender-i-p.component.html',
  styleUrls: ['./prev-calender-i-p.component.scss'],
  providers: [
    {
...
export class PrevCalenderIPComponent {

  /*
  ExecuteLabel1='<ul class="list-inline m-0"><li class="list-inline-item" style="padding:0px;margin:0px;margin-right:4px;"> <button ';
  ExecuteLabel4=' class="btn btn-default btn-xs rounded-0" type="button" data-toggle="tooltip" data-placement="top" title="afficher">  <span>Executer &nbsp;</span><i class="pg-icon md-18" style="height:10px;position:relative;top:1px;">flash</i> </button></li></ul>';
  ExecuteLabel=this.ExecuteLabel1+this.ExecuteLabel4;
  */
 ExecuteLabel='<i class="pg-icon md-18 calender-btn">flash</i>';
 AfficherLabel='<i class="pg-icon md-18 calender-btn">history</i>';
  ...
  events$: Observable<CalendarEvent<{ ip: IP }>[]>;
  ...
  constructor(private interventionService:InterventionService,private matDialog: MatDialog,private _router: Router) {}
  ...
  fetchEvents(): void {
    ...
    this.events$ = this.interventionService.getEventsIPs(body)
    //.subscribe(res=>{console.log(res)})
      ...
      .pipe(
          ...
          //console.log(results);
          return results.map((ip: any) => {
            return {
              //title: '<b>'+ip.time.slice(0,-3)+'</b> '+ip.tache,
              title: ip.tache,
              start: new Date( ip.date_resultat
                //di.date + getTimezoneOffsetString(this.viewDate)
              ),
              color: this.PickColor(ip.decalage),
              allDay: true,
              meta: {
                ip,
              },
              actions: [
                {
                  label:this.ExecuteLabel,
                  //label: '<span> Edit </span>',
                  //label: '<ul class="list-inline m-0"><li class="list-inline-item" style="padding:0px;margin:0px;margin-right:4px;"> <button [routerLink]="[/gmao/preventif/otp/add/,data.InterventionID,0]" class="btn btn-default btn-xs rounded-0" type="button" data-toggle="tooltip" data-placement="top" title="afficher">  <span>Executer &nbsp;</span><i class="pg-icon md-18" style="height:10px;position:relative;top:1px;">flash</i> </button></li> ',
                  ...
                  onClick: ({ event }: { event: CalendarEvent }): void => {
                    //this.planifierDi(event.meta.ip.InterventionID,event.meta.ip.InterventionID);
                    //console.log('(E)');
                    this.ExecuterIP(event.meta.ip.InterventionID);
                  },
                },
                {
                  label:this.AfficherLabel,
                  onClick: ({ event }: { event: CalendarEvent }): void => {
                    //this.deplanifierDi(event.meta.di.PlanID);
                    //console.log('(X)');
                   
                    this.reporterPlandialog(event.meta.ip.date_resultat,event.meta.ip.InterventionID);
                  },
                }
  ...
  eventClicked(event: CalendarEvent<{ ip: IP }>): void {

    this.AfficherPlan(event.meta.ip.InterventionID);
...
ExecuterIP(InterventionID:number){
  let url = "/gmao/preventif/otp/add/"+InterventionID+"/0";
...
deplanifierDi(PlanID:number){
  /*
  this.interventionService.deplanifierDi(PlanID).then(
    res=>{ 
       console.log(res); 
       //this.events$=null;
       this.fetchEvents();
...
openIP(InterventionID:number){
  const dialogConfig = new MatDialogConfig();
      dialogConfig.panelClass="custom-dialog-container";
      dialogConfig.autoFocus = true;
      dialogConfig.width = "80%";
      dialogConfig.data=InterventionID;
      this.matDialog.open(PrevPlanComponent, dialogConfig)
      .afterClosed().subscribe( res=> {});
}

AfficherPlan(InterventionID:number){
  //console.log('Affichage Plan :'+PlanID);
  this.openIP(InterventionID);
}

  PickColor(decalage){
    if(decalage==0){return  colors.blue;}
    if(decalage!=0){return  colors.yellow;}
  ...
  reporterPlandialog(date:string,InterventionID:number) {
    ...
    this.matDialog.open(SharedReporterDateComponent, dialogConfig).afterClosed().subscribe(res => {
     if(res != undefined && res != null){
