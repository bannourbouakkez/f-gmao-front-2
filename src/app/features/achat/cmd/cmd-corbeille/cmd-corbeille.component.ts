...
@Component({
  selector: 'app-cmd-corbeille',
  templateUrl: './cmd-corbeille.component.html',
  styleUrls: ['./cmd-corbeille.component.scss']
})
export class CmdCorbeilleComponent implements OnInit {

  restaurerCmd(ComandeID:number,i){
    this.cmdService.restaurerCmd(ComandeID).then(
      res=>{
        this.cmds.splice(i, 1);
        this.notification('success', "Commande est restauré avec succées .", 1500);
      },
 ...
 breadcrumb=[
  ...
  {url:'',title:'Liste Commandes supprimées'}
];
