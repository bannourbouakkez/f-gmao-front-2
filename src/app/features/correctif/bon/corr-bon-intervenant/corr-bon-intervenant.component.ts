...
@Component({
  selector: 'app-corr-bon-intervenant',
  templateUrl: './corr-bon-intervenant.component.html',
  styleUrls: ['./corr-bon-intervenant.component.scss']
})
export class CorrBonIntervenantComponent implements OnInit {
  ...
  errorMsgs=<any>[];
   ...
   search2(value: string): void {
     ...
     this.isLoading2=true; 
      ...
      this.corrBonService.getTachesBySearchWordByEquipementID(value,this.data.EquipementIDForTaches).then(
         data=>{
         console.log(data);
         this.tacheList = data;
         this.isLoading=false;
  ...
  */
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data,
    public dialogRef: MatDialogRef<CorrBonIntervenantComponent>,
    private corrBonService: CorrBonService,private _notification: MessageService
  ...
  ngOnInit() {
    ...
    if (this.data.BonIntervenantIndex == null) {
      ...
      this.formData = Object.assign({}, this.corrBonService.bonintervenants[this.data.BonIntervenantIndex]);
    }


    this.search2('');

    this.formDataDatesTimesChangedFunc(); // 5
...
*/
  ...
  onSubmit(form: NgForm) {
    ...
    if (this.isFull(form)) {
      if (this.TesterTimeMinTimeMax(form.value.date1, form.value.time1, form.value.date2, form.value.time2)) {
        ...
        if (this.TesterMemeIntervenantMemeTacheTempsCommune(form.value.BonIntervenantID, form.value.intervenant_id, form.value.tache_id, form.value.date1, form.value.time1, form.value.date2, form.value.time2)) {
          if (this.data.BonIntervenantIndex == null) {
            this.corrBonService.bonintervenants.push(form.value);
          } else {
            this.corrBonService.bonintervenants[this.data.BonIntervenantIndex] = form.value;
          }
        } else {
          success = false;
  ...
  */
  ...
  updateName(ctrl) {
    ...
    this.formDataDatesTimesChangedFunc(); // 2
  }
  ...
  updateTache(ctrl) {
    ...
    this.formDataDatesTimesChangedFunc(); // 3
  }
  ...
  isFull(form:NgForm){
     ...
     || this.isNOrU(form.value.intervenant_id) || this.isNOrU(form.value.tache_id) ) { return false; }
     return true;
  }
  ...
  TesterMemeIntervenantMemeTacheTempsCommune(BonIntervenantID: number, intervenant_id: number, tache_id: number, date1, time1, date2, time2) {
    ...
    for (let i = 0; i < this.corrBonService.bonintervenants.length; i++) {
      let _BonIntervenantID = this.corrBonService.bonintervenants[i].BonIntervenantID;
      let _intervenant_id = this.corrBonService.bonintervenants[i].intervenant_id;
      let _tache_id = this.corrBonService.bonintervenants[i].tache_id;
      ...
      let _date1 = new Date(moment(this.corrBonService.bonintervenants[i].date1).format("YYYY-MM-DDTHH:mm:00"));
      let _time1 = new Date(moment(this.corrBonService.bonintervenants[i].time1).format("YYYY-MM-DDTHH:mm:00"));
      let _date2 = new Date(moment(this.corrBonService.bonintervenants[i].date2).format("YYYY-MM-DDTHH:mm:00"));
      let _time2 = new Date(moment(this.corrBonService.bonintervenants[i].time2).format("YYYY-MM-DDTHH:mm:00"));

      //console.log(BonIntervenantID+'!='+_BonIntervenantID);
      /*//++
      if(this.data.BonID){ 
  
      if(BonIntervenantID==null || BonIntervenantID==undefined){ // lo5rin mriglin // ajouter nouveau
  
        if((_intervenant_id==intervenant_id) && (_tache_id == tache_id)){
          console.log('1 : '+BonIntervenantID+'!='+_BonIntervenantID);
        ...
        if( (_intervenant_id==intervenant_id) && (_tache_id == tache_id) && (BonIntervenantID!=_BonIntervenantID)){
          console.log('2 : '+BonIntervenantID+'!='+_BonIntervenantID);
          let isTempsCommune = this.isTempsCommune(date1,time1,date2,time2,_date1,_time1,_date2,_time2);
          if(isTempsCommune){return false;}
        }
...
formDataDatesTimesChangedFunc(){
 ...
 CheckIfFormDataValid(){
  ...
  && this.TesterMemeIntervenantMemeTacheTempsCommune(this.formData.BonIntervenantID,this.formData.intervenant_id,this.formData.tache_id,this.formData.date1,this.formData.time1,this.formData.date2,this.formData.time2)
  ) {
    ...
    if(!this.TesterMemeIntervenantMemeTacheTempsCommune(this.formData.BonIntervenantID,this.formData.intervenant_id,this.formData.tache_id,this.formData.date1,this.formData.time1,this.formData.date2,this.formData.time2)){  this.errorMsgs.push('famma 7aja commune'); }
  }
 }
